...
def game_main():
    ...
    if index == 0:  # タイトルロゴ
        ...
        draw_txt("Click to start.", 312, 708, 40, "white", "TITLE")
        index = 1
        mouse_c = 0
    elif index == 1:    # タイトル画面　スタート待ち
        difficulty = 0
        if mouse_c == 1:
            if 160 < mouse_x and mouse_x < 456 and 384 < mouse_y and mouse_y < 456:
                difficulty = 4
            if 160 < mouse_x and mouse_x < 456 and 528 < mouse_y and mouse_y < 600:
                difficulty = 5
            if 160 < mouse_x and mouse_x < 456 and 672 < mouse_y and mouse_y < 744:
                difficulty = 6
        if difficulty > 0:
            for y in range(10):
                for x in range(8):
    ...
    if tsugi > 0:
        cvs.create_image(752, 120, image=img_neko[tsugi], tag="INFO")
...
cursor = tkinter.PhotoImage(file=FILE+"neko_cursor.png")
img_neko = [
    None,
